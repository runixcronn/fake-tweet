...
const ImagesContainer = ({ image }) => {
  switch (image.length) {
    ...
    case 2:
      return (
        <div className="mt-2 mb-2 w-full h-80 rounded-2xl border border-gray-300 overflow-hidden">
          <OneImageColumn side="left" image={image[0]} />
          <div className="float-left w-0.5 h-full" />
...
const OneImageColumn = ({ side, image }) => {
  ...
  const borderSide = side.charAt(0);

  return (
    <div
      className={`float-left w-1/2 h-full overflow-hidden ${
        side === "left" ? "rounded-l-2xl" : "rounded-r-2xl"
      }`}
    >
      <img src={image} alt="" className="h-full min-w-full object-cover" />
    </div>
  );
};
